...
func (c *Computer) nextInstructionPart1() error {
...
func (c *Computer) nextInstructionPart2() error {
	c.instruction = c.read(1)
	opcode := c.instruction % 100
	switch opcode {
	case 99:
...
func buildComputer(input int) Computer {
	...
	computer := Computer{&memory, input, 0, 0, -1}
	return computer
}

// Run day 5
func part1() int {
	computer := buildComputer(1)

	for {
		err := computer.nextInstructionPart1()
		if err != nil {
			// fmt.Println(err)
			break
		}
...
func part2() int {
	computer := buildComputer(5)

	for {
		err := computer.nextInstructionPart2()
		if err != nil {
			// fmt.Println(err)
			break
		}
...
func Run() {
	fmt.Println("-- Day 5 --")
	fmt.Println("Part one output:", part1())
	fmt.Println("Part one output:", part2())
}
